...
SENTRY_WEBHOOK: Optional[WebhookConfig] = next(
    (c for c in _loaded_configs if c.name == "sentry"), None
)
...
def reload_webhook(name: str) -> Optional[WebhookConfig]:
    ...
    global GITHUB_WEBHOOK, JIRA_WEBHOOK, SLACK_WEBHOOK, SENTRY_WEBHOOK
...
def reload_all_webhooks() -> List[WebhookConfig]:
    ...
    SENTRY_WEBHOOK = next((c for c in _loaded_configs if c.name == "sentry"), None)
