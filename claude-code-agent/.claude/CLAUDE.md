...
├── testing/             # TDD phases
├── code-refactoring/    # Systematic refactoring patterns
...
When code has duplication, long functions, high complexity, or placeholder functions:

- Use `code-refactoring` skill for systematic improvements
- Always write tests first if missing
- Refactor incrementally (one change at a time)
- Verify behavior preserved after each change
...
- High complexity (>10 decision points) → Simplify Conditionals
- Code duplication → Extract Method/Class
- Placeholder functions → Implement or Remove (YAGNI)
